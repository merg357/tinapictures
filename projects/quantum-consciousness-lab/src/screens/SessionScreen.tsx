...
export function SessionScreen({ plan, onExit, onComplete }: { plan: SessionPlan; onExit: () => void; onComplete: (record: SessionRecord) => void }) {
  ...
  function saveAndFinish() {
    onComplete({
      ...
      pathId: plan.pathId,
      title: plan.title,
      minutes: Math.max(1, Math.round(elapsed / 60)) || plan.minutes,
      before,
      after,
      completed: finished || elapsed >= 30,
      createdAt: Date.now(),
    });
  }
  ...
  const breathActive = /(breath|settle|regulate|arrive|exhale)/i.test(current.title);
  ...
  if (reflecting) {
    return <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.bg }}><ScrollView contentContainerStyle={{ padding: 22, paddingTop: 42, paddingBottom: 40 }}>
