...
function nearestSupportedMinutes(value) {
  ...
  return SUPPORTED_MINUTES.reduce((best, candidate) =>
...
function choosePath(intent = '', goal = '') {
  ...
  if (/(intention|manifest|visuali[sz]e|future|goal)/.test(text)) return PATHS.intention;
  if (/(quantum|entangle|superposition|observer effect|bohm)/.test(text)) return PATHS['quantum-foundations'];
  if (/(expanded|altered|hypnagog|out.of.body|deep state|consciousness)/.test(text)) return PATHS['expanded-consciousness'];
  if (/(argument|replay|overthink|thought|ruminat|angry|let it go|observer|reset)/.test(text)) return PATHS.observer;
...
function lensNote(lens) {
  if (lens === 'full') {
    return 'Includes clearly labeled philosophical and spiritual interpretations alongside the scientific framing.';
  }
  if (lens === 'science-frontier') {
    return 'Includes established science plus clearly labeled frontier hypotheses where relevant.';
  }
...
function composeSession({ intent = '', goal = '', minutes = 10, lens = 'science' } = {}) {
  ...
  return {
    id: `${path.pathId}-${normalizedMinutes}`,
    pathId: path.pathId,
    title: path.title,
    ...
    segments: path.segmentNames.map((name, index) => ({
      id: `${path.pathId}-${index + 1}`,
      title: name,
      minutes: allocations[index],
      prompt: buildPrompt(path.pathId, name, lens),
    })),
  };
}
...
function buildPrompt(pathId, segmentName, lens) {
  const prompts = {
    ...
    'quantum-foundations': 'Observe experience carefully. The act of psychological observation can change your relationship to a thought; this is an analogy, not a claim about quantum measurement in the brain.',
  ...
  const base = prompts[pathId] || prompts.coherence;
  return segmentName === 'Arrive' ? 'Get comfortable, notice the support beneath you, and allow the next few breaths to become natural.' : `${base} ${lensNote(lens)}`;
}

module.exports = {
  composeSession,
  nearestSupportedMinutes,
  choosePath,
  allocateMinutes,
};
